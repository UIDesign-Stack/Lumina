...
function emptyForm() {
  return {
    ...
    formatExport: 'pdf',
    sertakanRingkasan: true,
    sertakanProyeksi: true,
    sertakanDetailTransaksi: false,
    catatan: '',
  }
}
...
function validate() {
  ...
  if (!form.jenisAktivitas) errors.jenisAktivitas = 'Jenis aktivitas wajib dipilih'
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <div>
                <h2 class="text-lg font-semibold" :class="isDark ? 'text-white' : 'text-gray-900'">Buat Laporan Arus Kas</h2>
                <p class="text-xs mt-0.5" :class="isDark ? 'text-gray-400' : 'text-gray-500'">Susun laporan arus kas untuk periode yang kamu pilih.</p>
              </div>
              <button
                type="button"
            ...
            <form @submit.prevent="handleSubmit" class="px-6 py-5 space-y-5">
              ...
              <div>
                ...
                <input
                  v-model="form.nama"
                  type="text"
                  placeholder="Contoh: Laporan Arus Kas Mei 2026"
              ...
              <div>
                ...
                <p v-if="errors.jenisAktivitas" class="text-xs text-rose-500 mt-1">{{ errors.jenisAktivitas }}</p>
              </div>
              ...
              <div>
                ...
                <div class="space-y-2.5">
                  <label class="flex items-center gap-2.5 text-sm cursor-pointer" :class="isDark ? 'text-gray-300' : 'text-gray-700'">
                    <input v-model="form.sertakanRingkasan" type="checkbox" class="w-4 h-4 rounded accent-purple-600" />
                    Sertakan ringkasan arus kas
                  </label>
                  <label class="flex items-center gap-2.5 text-sm cursor-pointer" :class="isDark ? 'text-gray-300' : 'text-gray-700'">
                    <input v-model="form.sertakanProyeksi" type="checkbox" class="w-4 h-4 rounded accent-purple-600" />
                    Sertakan proyeksi arus kas 30 hari ke depan
                  </label>
                  <label class="flex items-center gap-2.5 text-sm cursor-pointer" :class="isDark ? 'text-gray-300' : 'text-gray-700'">
                    <input v-model="form.sertakanDetailTransaksi" type="checkbox" class="w-4 h-4 rounded accent-purple-600" />
                    Sertakan detail per transaksi kas
                  </label>
                  <label class="flex items-center gap-2.5 text-sm cursor-pointer" :class="isDark ? 'text-gray-300' : 'text-gray-700'">
