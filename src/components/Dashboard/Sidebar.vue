...
<template>
  ...
  <aside
    class="fixed lg:sticky top-0 left-0 h-screen w-64 flex flex-col z-50 transition-colors duration-300 lg:translate-x-0"
    :class="[
  ...
  >
    ...
    <div class="flex items-center justify-between px-5 py-5">
      <div class="flex items-center gap-2.5">
        ...
        <div>
          <p :class="isDark ? 'text-white' : 'text-gray-900'" class="font-semibold tracking-[0.15em] text-sm leading-none">LUMINA</p>
        ...
        <ul class="space-y-1">
          <li v-for="item in group.items" :key="item.to">
            <router-link v-slot="{ isExactActive, navigate, href }" :to="item.to" custom>
              <a
                :href="href"
                class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition cursor-pointer"
                :class="isExactActive
