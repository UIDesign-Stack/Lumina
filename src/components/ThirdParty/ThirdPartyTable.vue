...
import { UserIcon, BuildingOffice2Icon, EllipsisVerticalIcon, ChevronLeftIcon, ChevronRightIcon } from '@heroicons/vue/24/outline'
...
const iconMap = { user: UserIcon, building: BuildingOffice2Icon }
const iconColorMap = {
...
const typeStyles = {
  ...
  Pelanggan: 'bg-blue-500/15 text-blue-400',
  Konsultan: 'bg-amber-500/15 text-amber-400',
  Lainnya: 'bg-sky-500/15 text-sky-400',
}
const statusStyles = {
...
const rangeLabel = computed(() => {
  ...
  return `Menampilkan ${start} - ${end} dari ${props.rows.length} pihak ketiga`
})
  ...
  <div class="rounded-2xl border" :class="isDark ? 'bg-[#100c1c]/80 border-white/10' : 'bg-white border-gray-200'">
    ...
    <div class="overflow-x-auto hidden lg:block">
      <table class="w-full text-sm">
        <thead>
          <tr>
            <th class="text-left px-5 py-3 font-medium" :class="isDark ? 'text-gray-400' : 'text-gray-500'">Nama Pihak Ketiga</th>
            <th class="text-left px-3 py-3 font-medium" :class="isDark ? 'text-gray-400' : 'text-gray-500'">Tipe</th>
            <th class="text-left px-3 py-3 font-medium" :class="isDark ? 'text-gray-400' : 'text-gray-500'">Kontak</th>
            <th class="text-left px-3 py-3 font-medium" :class="isDark ? 'text-gray-400' : 'text-gray-500'">Email</th>
            <th class="text-right px-3 py-3 font-medium" :class="isDark ? 'text-gray-400' : 'text-gray-500'">Total Transaksi</th>
          ...
          >
            <td class="px-5 py-3">
              <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0" :class="iconColorMap[row.color] || iconColorMap.gray">
                  <component :is="iconMap[row.icon] || iconMap.user" class="w-4.5 h-4.5" />
                </div>
                <div class="min-w-0">
                  <p class="font-medium whitespace-nowrap" :class="isDark ? 'text-gray-200' : 'text-gray-800'">{{ row.name }}</p>
                  <p class="text-xs" :class="isDark ? 'text-gray-500' : 'text-gray-400'">{{ row.subtitle }}</p>
                </div>
              </div>
            </td>
            <td class="px-3 py-3">
            ...
            <td class="px-3 py-3 whitespace-nowrap" :class="isDark ? 'text-gray-300' : 'text-gray-600'">
              <p>{{ row.contact }}</p>
              <p v-if="row.phone && row.phone !== '-'" class="text-xs" :class="isDark ? 'text-gray-500' : 'text-gray-400'">{{ row.phone }}</p>
            </td>
            <td class="px-3 py-3 whitespace-nowrap" :class="isDark ? 'text-gray-400' : 'text-gray-500'">{{ row.email }}</td>
            <td class="px-3 py-3 text-right whitespace-nowrap">
              <p class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">{{ formatAmount(row.totalTransaction) }}</p>
              <p class="text-xs" :class="isDark ? 'text-gray-500' : 'text-gray-400'">{{ row.transactionCount }} transaksi</p>
            </td>
            <td class="px-3 py-3">
      ...
      <div v-for="row in paginatedRows" :key="row.id" class="p-4">
        <div class="flex items-start gap-3 mb-2">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs" :class="isDark ? 'text-gray-500' : 'text-gray-400'">{{ row.email }}</p>
          </div>
        </div>
        <div class="flex items-center justify-between text-xs" :class="isDark ? 'text-gray-500' : 'text-gray-400'">
          <span class="px-2 py-0.5 rounded-full text-[11px] font-medium" :class="typeStyles[row.type]">{{ row.type }}</span>
          <span>{{ formatAmount(row.totalTransaction) }} · {{ row.transactionCount }} trx</span>
        </div>
      </div>
    </div>
