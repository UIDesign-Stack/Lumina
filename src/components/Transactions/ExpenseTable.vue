...
const filteredRows = computed(() => {
  ...
  return props.rows.filter((r) =>
    [r.description, r.category, r.account, r.createdBy].some((field) => field.toLowerCase().includes(q))
  )
})
...
<template>
  <div class="rounded-2xl border" :class="isDark ? 'bg-[#100c1c]/80 border-white/10' : 'bg-white border-gray-200'">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-5">
      <p class="text-base font-semibold" :class="isDark ? 'text-white' : 'text-gray-900'">Daftar Pengeluaran</p>
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-t" :class="isDark ? 'border-white/5' : 'border-gray-100'">
            ...
            <th class="text-left px-3 py-3 font-medium" :class="isDark ? 'text-gray-400' : 'text-gray-500'">Akun</th>
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-3 py-3 text-right font-medium text-rose-400 whitespace-nowrap">{{ formatAmount(row.amount) }}</td>
            <td class="px-3 py-3">
              <span class="inline-flex items-center gap-1 text-xs font-medium" :class="statusStyles[row.status]">
                <CheckCircleIcon v-if="row.status === 'Selesai'" class="w-3.5 h-3.5" />
    ...
    <div class="md:hidden divide-y" :class="isDark ? 'divide-white/5' : 'divide-gray-100'">
      <div v-for="row in paginatedRows" :key="row.id" class="p-4">
        <div class="flex items-start justify-between mb-1.5">
          ...
          <p class="text-sm font-medium text-rose-400 flex-shrink-0 ml-2">{{ formatAmount(row.amount) }}</p>
        </div>
        <div class="flex items-center gap-2 mb-2">
          ...
          <span class="text-xs" :class="isDark ? 'text-gray-500' : 'text-gray-400'">{{ row.createdBy }}</span>
        </div>
        <div class="flex items-center justify-between text-xs" :class="isDark ? 'text-gray-500' : 'text-gray-400'">
          ...
          <span class="inline-flex items-center gap-1 font-medium" :class="statusStyles[row.status]">
            <CheckCircleIcon v-if="row.status === 'Selesai'" class="w-3.5 h-3.5" />
            <ClockIcon v-else class="w-3.5 h-3.5" />
            {{ row.status }}
          </span>
