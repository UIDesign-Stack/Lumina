...
function validate() {
  ...
  if (!form.sumber) errors.sumber = 'Sumber pemasukan wajib diisi'
  if (!form.metodePembayaran) errors.metodePembayaran = 'Metode pembayaran wajib dipilih'
  if (!form.akunTujuan) errors.akunTujuan = 'Akun tujuan wajib dipilih'
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <div>
                <h2 class="text-lg font-semibold" :class="isDark ? 'text-white' : 'text-gray-900'">Pemasukan Baru</h2>
                <p class="text-xs mt-0.5" :class="isDark ? 'text-gray-400' : 'text-gray-500'">Catat transaksi pemasukan baru ke dalam pembukuan.</p>
              </div>
              <button
                type="button"
            ...
            <form @submit.prevent="handleSubmit" class="px-6 py-5 space-y-5">
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label class="block text-sm font-medium mb-1.5" :class="isDark ? 'text-gray-300' : 'text-gray-700'">
                    Jumlah Pemasukan <span class="text-rose-500">*</span>
                  </label>
                  <div class="relative">
                    <span
                ...
                <div>
                  <label class="block text-sm font-medium mb-1.5" :class="isDark ? 'text-gray-300' : 'text-gray-700'">
                    Sumber Pemasukan <span class="text-rose-500">*</span>
                  </label>
                  <input
                    v-model="form.sumber"
                    type="text"
                    placeholder="Nama klien, perusahaan, dll."
                    class="w-full rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/60 transition"
                    :class="isDark
                      ? 'bg-[#181428]/80 border border-white/10 text-white placeholder-gray-500'
                      : 'bg-gray-50 border border-gray-200 text-gray-900 placeholder-gray-400'"
                  />
                  <p v-if="errors.sumber" class="text-xs text-rose-500 mt-1">{{ errors.sumber }}</p>
                </div>
                ...
                <div>
                  <label class="block text-sm font-medium mb-1.5" :class="isDark ? 'text-gray-300' : 'text-gray-700'">
                    Akun/Rekening Tujuan <span class="text-rose-500">*</span>
                  </label>
                  <select
                    v-model="form.akunTujuan"
                    class="w-full rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/60 transition"
                    :class="isDark
                      ? 'bg-[#181428]/80 border border-white/10 text-white'
                      : 'bg-gray-50 border border-gray-200 text-gray-900'"
                  ...
                  <p v-if="errors.akunTujuan" class="text-xs text-rose-500 mt-1">{{ errors.akunTujuan }}</p>
                </div>
