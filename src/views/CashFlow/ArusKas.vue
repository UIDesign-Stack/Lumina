...
<template>
  <DashboardLayout :unread-notifications="1">
    ...
    <div class="flex flex-wrap items-start justify-between gap-4 mb-6">
      <div>
        <h1 class="text-2xl font-bold" :class="isDark ? 'text-white' : 'text-gray-900'">Laporan Arus Kas</h1>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-4">
          <div class="lg:col-span-3 rounded-2xl border p-5" :class="isDark ? 'bg-[#100c1c]/80 border-white/10' : 'bg-white border-gray-200'">
            <FinancialTrendChart
              ...
              :labels="trend.labels"
              :series="[
                { label: 'Kas Masuk', data: trend.cashIn, color: '#a855f7' },
                { label: 'Kas Keluar', data: trend.cashOut, color: '#f43f5e' },
                { label: 'Arus Kas Bersih', data: trend.netCashFlow, color: '#3b82f6' },
              ]"
            />
          </div>
          <div class="lg:col-span-2 rounded-2xl border p-5" :class="isDark ? 'bg-[#100c1c]/80 border-white/10' : 'bg-white border-gray-200'">
            <p class="text-base font-semibold mb-4" :class="isDark ? 'text-white' : 'text-gray-900'">Ringkasan Arus Kas</p>
            <CategoryDonutChart :categories="summaryBreakdown" :total="summaryTotal" />
          </div>
        </div>

        <CashFlowTable :tabs="tabs" :rows="rows" />
      </div>

      <div class="space-y-4">
        <CashActivityList :activities="activities" />

        <div class="rounded-2xl border p-5" :class="isDark ? 'bg-[#100c1c]/80 border-white/10' : 'bg-white border-gray-200'">
          <FinancialTrendChart
            title="Proyeksi Arus Kas (30 Hari ke Depan)"
            period-label="Mingguan"
            :labels="projection.labels"
            :series="[
              { label: 'Proyeksi Masuk', data: projection.cashIn, color: '#a855f7' },
              { label: 'Proyeksi Keluar', data: projection.cashOut, color: '#f43f5e' },
              { label: 'Proyeksi Saldo', data: projection.balance, color: '#3b82f6' },
            ]"
          />
        </div>
